...
- **当前版本**：v1.1.0（发布于 2025年5月30日）
- **版本说明**：
  - v1.1.0：初始版本，包含用户管理、医生/患者管理、排班管理、挂号预约、病历管理和支付功能。
- **版本控制工具**：建议使用 Git 进行版本管理，项目代码可通过 `git clone <项目地址>` 获取。

## 运行环境要求

- **Python 版本**：推荐使用 Python 3.10 或更高版本（建议 3.12），确保兼容性并支持所有依赖库。
- **依赖库**：请参考 `requirements.txt` 安装所需包（如 Flask、Flask-SQLAlchemy 等）。
- **数据库**：需安装 MSSQL ODBC Driver 17（用于 MSSQL）或 SQLite 支持。
